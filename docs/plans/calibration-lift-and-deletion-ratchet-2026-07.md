...
1. **Coverage invariant** (do this first): *no calibratable agent without a plant.* One harness
   ...
   `integration-adversary` — that's the point. When the closing pair is authored, its `plant`
   description should name the meta-class explicitly: the agent whose job is finding uncovered
   things was itself the uncovered thing — §6a old-blind-to-new at the meta level; the
   coverage-checker must be coverage-checked.
...
| **4** | **Lift diagnostic** — a control arm, run once, quarterly, on holdout. **Design change:** a *naive* bare-agent arm is confounded. Roughly 17 of 24 oracles key on house output contracts (`NOT VERIFIED`, `RED-FIRST: VERIFIED`, `verdict: band-aid`, `parking: DARK`, `Tripwire: 2/2`, `coverage: adequate`, `Recommendation:`, `confirmed: 0`). A bare agent can diagnose a defect perfectly and still miss `must_match` because it doesn't know the format — so the arm would measure "knows the house format," overstating lift almost everywhere. The confound is **bidirectional**: on controls, whose oracles key on `gate passes`, a bare agent also fails, so FP is equally uninterpretable. Fix: a **contract-stub** control prompt carrying the output format only, no doctrine — and it must cover the *control* verdict lines too, not just the plant ones. **Stub-freeze rule (answers open question 5):** the stub is *measurement apparatus, not prompt engineering* — ONE shared, frozen, minimal stub (verdict-line grammar only, nothing about how to investigate), versioned, its hash recorded in every lift report, protected under the same append-only-with-journal regime as the oracles, because a stub edit moves the measurement floor. If a per-agent stub ever seems necessary, that is evidence the *oracle* is over-anchored on format — fix the oracle (journal entry), never grow the stub. Stub growth between quarterly runs is then a visible drift signal, and the collapse mode ("the stub becomes the doctrine") is structurally impossible rather than vigilance-dependent. | **Real but indirect, gated on 2 + 3.** Value is *finding decoration*, not deleting things: dead prose isn't neutral, it eats context and competes with the instructions that do steer. Safe to look at only once cutting is expensive. |
...
### v3 — third review pass folded (2026-07-30)

The reviewer re-verified v2's counter-claims against the live repo rather than accepting the
response prose — and recorded their own misses with the same candor demanded of the draft:

| Item | Outcome |
|---|---|
| `integration-adversary` is the sole uncovered calibratable agent | **Confirmed by join enumeration** (roster − tree-touching − agents-used-in-scenarios∪corpus = exactly `{integration-adversary}`) — not inferred from the mechanism this time. |
| The paper review is committed on a branch | **Confirmed via `git ls-remote`** — `claude/paper-review-2607-2dqbql` exists on origin. |
| Oracle-format count | Reviewer's independent sweep: **20 of 24** vs the draft's ~17 — wider regex net, same direction; the discrepancy is net-width, not substance. Either number sustains the confound. |
| Reviewer's own error class, recorded | Both of the reviewer's failed v1 claims failed the same way: **under-sweeping the stated universe** — verifying the mechanism instead of enumerating the join ("8 of 10"), and sweeping `main` while calling it "the repo" (the branch). A negative claim's sweep must cover the universe as stated, per §12. |
| Open question 5 (per-agent stubs) | **Answered — stub-freeze rule**, folded into item 4: one shared frozen stub as protected measurement apparatus; per-agent pressure means fix the oracle, never grow the stub. |
| Meta-plant framing | Folded into R1 part 1: the coverage-checker must be coverage-checked (§6a at the meta level), named in the closing scenario's `plant` description. |

Open questions 1, 2, and 5 are now answered (class-level grain; separate journal file;
stub-freeze). Remaining:
...
v3 re-verification (reviewer, same `89abcbc` HEAD): uncovered-agent set computed by join
enumeration → exactly `{integration-adversary}`; `claude/paper-review-2607-2dqbql` present on
origin via `ls-remote`; independent oracle-format sweep → 20/24 (draft's ~17/24 — net-width
difference only); `calibration` string in `test_installer.py` is docstring-incidental — **no
vendoring pin exists**, R2's test is genuinely missing.
