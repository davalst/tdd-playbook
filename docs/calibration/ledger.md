...
| L-20260909-01 | 2026-09-09 | 968fbe3 | plugins/tdd-playbook/agents/mutation-runner.md | v1.52.0: the pass is invoked with `--scope <scope-name>` from the mapping (no --suite-args); with_snapshot begin/verify rescoped to passes outside the runner; the roster-gap sentence gains the explicit-mapping clause; the three-bucket accounting restatement sits BESIDE `killed + survived < generated`, which is kept verbatim because the `unmeasured-not-certified` oracle anchors on it | vacuous-mutation-scope, red-baseline-false-green, unmeasured-not-certified, mutation-phantom-run, control-cachebusted-run, control-real-scope-measured, control-green-baseline-measured, control-accounting-reconciles | none (no change on all 8) | unclaimed — calibration is opt-in; the oracle regex is untouched (scoreboard integrity) | the brief's refusal vocabulary is unchanged where the oracles read it; only the invocation shape and two doctrine sentences moved |
