...
| L-20260831-02 | 2026-08-31 | fe5a519 | plugins/tdd-playbook/commands/grade.md | /grade must read `escape_log.py report` FIRST and paste it verbatim — the discovery-path split (§13's WHO CAUGHT IT) gains its first code reader after an exhaustive sweep found it written in one place and read by none | — | none | 0 | the instrument is new and its denominator is 9 seeded rows; a yield prediction on one cycle of one operator's data would be a fabricated number. Re-register with a real prediction once a second cycle exists |
| L-20260831-03 | 2026-08-31 | fe5a519 | plugins/tdd-playbook/agents/adoption-adversary.md;plugins/tdd-playbook/agents/architecture-adversary.md;plugins/tdd-playbook/agents/claims-verifier.md;plugins/tdd-playbook/agents/control-quality-adversary.md;plugins/tdd-playbook/agents/edge-case-adversary.md;plugins/tdd-playbook/agents/integration-adversary.md;plugins/tdd-playbook/agents/intent-adversary.md;plugins/tdd-playbook/agents/mutation-runner.md;plugins/tdd-playbook/agents/observability-adversary.md;plugins/tdd-playbook/agents/planted-error-probe.md;plugins/tdd-playbook/agents/red-first-verifier.md;plugins/tdd-playbook/agents/script-adversary.md;plugins/tdd-playbook/agents/security-adversary.md;plugins/tdd-playbook/agents/test-quality-adversary.md;plugins/tdd-playbook/agents/tripwire-auditor.md;plugins/tdd-playbook/agents/ux-probe-calibrator.md | COMMIT-FIRST CLAUSE ADDED THEN REVERTED IN THE SAME CYCLE. The 12 judging briefs gained a 'Commit before you read' section and a `Prior:` forced line, then D3's replay scored arm A (anchored) 3/3 and arm B (commit-first) 3/3 on the three plans this repo rejected, so the pre-registered rule (`B <= A` reverts) fired and the briefs were restored byte-exact to v1.48.0. Registered because the surfaces MOVED during the cycle even though the net diff is zero — an unregistered round trip is indistinguishable from never having tried | — | none | 0 | the change is gone; there is nothing left to score. Recorded so the attempt and its refutation are visible to the next author, who would otherwise re-propose it. The REASON it scored 0 is a ceiling in my experiment (arm A at 3/3 had no headroom), documented in docs/calibration/2026-08-31-commit-first-replay-RESULTS.md — that is a flaw in the test, not evidence for the clause, and the clause stays out until a subject set with real arm-A misses exists |
