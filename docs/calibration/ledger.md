...
The commit-first clause was BUILT and then REVERTED in the same cycle, so it registers no
surface change — the briefs are byte-identical to v1.48.0. What survives is the SKILL.md rule
and the escape ledger's reader. Both are prose/report surfaces with no scenario that scores
them, so `expect: none` again, and stated as unmeasurable rather than claimed as measured.

| id | date | baseline_sha | surface | change | scenarios | expect | claimed | rationale |
|---|---|---|---|---|---|---|---|---|
| L-20260831-01 | 2026-08-31 | fe5a519 | plugins/tdd-playbook/skills/tdd-playbook/SKILL.md | §1 gains the text-is-PRODUCT vs text-DESCRIBES-the-product rule with a one-question test ("would this still match if the implementation were deleted and only the comment remained?") and a preference order — run it, else match the assignment/call, else assert the substitution count, strip comments only where a text scan is unavoidable. Pays debt `proxy-assert-on-own-prose` after six occurrences in one day across two repos | — | none | 0 | doctrine yield is deliberately unmeasured (2026-08-14); no scenario models a vacuous substring assertion. The in-repo instance it fixed (test_verify_verdict.py:188) IS mechanically pinned, which is where the real assurance sits |
| L-20260831-02 | 2026-08-31 | a64215a | plugins/tdd-playbook/commands/grade.md | /grade must read `escape_log.py report` FIRST and paste it verbatim — the discovery-path split (§13's WHO CAUGHT IT) gains its first code reader after an exhaustive sweep found it written in one place and read by none | — | none | 0 | the instrument is new and its denominator is 9 seeded rows; a yield prediction on one cycle of one operator's data would be a fabricated number. Re-register with a real prediction once a second cycle exists |

L-20260831-03 was DROPPED, not renamed: it registered the commit-first briefs as a
changed surface, but the revert restored them BYTE-EXACT to v1.48.0, so no gate surface
moved and there is nothing to predict. Agent briefs are EFFECTFUL and cannot carry
`expect: none` — the ledger was right to refuse it. The round trip is recorded where it
belongs, in docs/calibration/2026-08-31-commit-first-replay-RESULTS.md.

| L-20260909-01 | 2026-09-09 | df91a50 | plugins/tdd-playbook/agents/mutation-runner.md | v1.52.0: the pass is invoked with `--scope <scope-name>` from the mapping (no free-form suite args); the roster-gap sentence gains the explicit-mapping clause; the three-bucket accounting restatement sits BESIDE `killed + survived < generated`, kept verbatim because the oracle anchors on it | vacuous-mutation-scope; unmeasured-not-certified | up | 1 | the brief now names a mechanical refusal for both plants (a scope that resolves to nothing is refused by the mapping; unscored mutants refuse certification with the bucket named), so a correct refusal should be phrased in the house words more reliably; the other six mutation-runner scenarios are expected flat, and calibration is opt-in so the claim stays unscored until a run is made |
| L-20260910-01 | 2026-09-10 | b01b8ab | plugins/tdd-playbook/agents/spec-producer.md; calibration/scenarios.json | v1.53.0 §0a elicitation: a NEW 17th roster brief (`spec-producer`) that GATHERS instead of reviewing — it reads a request before the plan and returns a question pack, each question carrying a PROPOSED falsifiable answer, with the stopping rule set at DECISION CHANGE rather than uncertainty and the lenses bounded while the count never is; plus its paired plant/control in scenarios.json | producer-invented-spec; control-producer-specified | up | 3 | UP FROM NO MEASUREMENT, stated plainly so the direction is not read as a comparison that does not exist: this agent and both scenarios are new at this sha, so there is no prior rate and the claim being registered is that the FIRST run reaches k/k on BOTH halves. The control is the half worth pre-registering — this repo's own history records a verifier at recall 8/10 with FP 10/10 where the FP number was substantially measuring CONTROL-AUTHORING quality, so the control request was deliberately written LONG and complete (actor, authorization, boundary values, failure path, persisted effect, surface, explicit non-goals) rather than trivially short. The falsifiable prediction: if `control-producer-specified` comes back UNDERSPECIFIED, the spec-producer pads and the stopping rule is not being applied — that is a REGRESSED verdict against this entry and the fix is the brief, never the control. Calibration is opt-in, so the claim stays unscored until a run is made |
| L-20260910-02 | 2026-09-10 | b01b8ab | plugins/tdd-playbook/skills/tdd-playbook/SKILL.md; plugins/tdd-playbook/commands/spec-producer.md; plugins/tdd-playbook/commands/tdd-plan.md | v1.53.0 §0a elicitation, the doctrine and command half: a new `## 0a` section between §0 and §1 carrying the collapse rule, the ask/answer split, the decision-change stopping rule and the empty-pack-is-correct norm; §0's existing spec-integrity bullet now points at §0a as the mechanism for the rule it already stated; a new `/spec-producer` command; and `/tdd-plan`'s Spec integrity block named as the pack's only consumer | — | none | 0 | `expect: none` is the honest and LEGAL answer here, not an evasion: SKILL.md and commands/ are deliberately outside EFFECTFUL (David, 2026-08-14 — doctrine yield is unmeasured by recorded decision), and none of these three files is read by any scenario oracle, so any movement attributed to them would be attribution I cannot support. The measurable half of this change is L-20260910-01, which is where the falsifiable claim lives |
