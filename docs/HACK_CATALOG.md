...
| H9 | overmock_guard fabricated-seam pattern (planted: test_hooks G2e pair) — **OPT-IN since v1.32.0 (`TDD_PLAYBOOK_HOOK_OVERMOCK=warn`); on a default install this row is doctrine, not a live mechanism** | §1 seam-fabrication rule; §6a assembly suite |
...
| H13 | two-directional calibration table per blocking guard, now ENFORCED family-wide: `test_hooks.test_blocking_guards_prove_both_directions` enumerates the blocking-by-default roster from the real registry and requires BOTH directions per member, with one shared out-of-jurisdiction corpus supplying the allow direction for every guard (v1.45 — before it, all four guards satisfied the bar by the author's diligence and nothing required it of the next one; planted: a fabricated blocking member REDs the sweep). Per-guard rows: test_hooks v1.28 ALLOW-DIRECTION block — FP1–FP4 + surviving block rows; exitcode_guard for the discarded-verdict cousin — **exitcode_guard is OPT-IN since v1.32.0; the calibration-table half is live, the discarded-verdict hook is not, by default** | §13 guard self-claims are unverified claims; guard_note + `blocks · accounted · UNACCOUNTED` makes a routed-around block visible |
| H14 | exhaustive_claim_guard (**OPT-IN since v1.32.0** — `TDD_PLAYBOOK_HOOK_EXHAUSTIVE=warn`; planted: test_hooks test_exhaustive_claim, both directions) | §12 violating-case line; §6c family parity enumerates from the REAL registry; NOT covered by §4 mutation score |
