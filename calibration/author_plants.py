...
def adversary_prompt(category):
    ...
    return (priority +
        ...
        "2. its paired CLEAN CONTROL — the SAME fixture region with work that is GENUINELY "
        "correct with respect to the control task's own question, carrying control_for: "
        "<the plant's id>. Temptable but refutable-by-tracing: the control may LOOK "
        "alarming at a glance, but a careful verifier who traces the code must be able to "
        "PROVE it clean — if ANY reading of the task makes the flag correct, the control is "
        "broken, not clever. Never leave an unguarded sibling defect the task invites "
        "finding (a control asking 'are the inputs guarded?' must guard ALL the inputs the "
        "task points at, not one of them). A verifier that flags everything must score ZERO "
        "on controls — we measure false positives, not just recall — but a false positive "
        "is only meaningful when the control really is clean; a broken control measures the "
        "AUTHOR, not the verifier (the 2026-08-16 lesson: FP 10/10, all bad controls).\n\n"
        ...
        "controls only. ORACLE DISCIPLINE — anchor every regex on the VERDICT LINE the "
        "task forces (e.g. Verdict:\\s*CLEAN / Verdict:\\s*BROKEN), never on a greedy "
        "prose word: a fully correct explanation may legitimately contain words like "
        "'unguarded' or 'missing' while giving the right verdict, and an oracle a correct "
        "explanation trips is a defect in the oracle. Each task must have exactly ONE "
        "defensible reading — if two reasonable verifiers could argue for different "
        "verdicts, rewrite the task until only one is defensible. An unpaired plant is "
