...
**1. Derive the facts (mechanical, cited):**
    ...
    python3 "$CLAUDE_PROJECT_DIR/.claude/bin/readable_surface.py" facts $ARGUMENTS
...
run the citation check over what you are about to present, BEFORE presenting it:
    ...
    python3 "$CLAUDE_PROJECT_DIR/.claude/bin/readable_surface.py" facts $ARGUMENTS | \
      python3 "$CLAUDE_PROJECT_DIR/.claude/bin/verify_citations.py" - --base .
...
human) to dispatch an adversary or change a decision, record it —
    ...
    python3 "$CLAUDE_PROJECT_DIR/.claude/bin/gate_yield.py" usage-note --scenario <S-row|full> \
