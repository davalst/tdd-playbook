...
def main():
    ...
    p = run("--sha", REAL_SHA, "--ledger", LEDGER, "--max-age-s", HUGE, "--no-tag")
    ...
    # green resolves immediately (no polling needed)
    slept2 = []
    ok, _ = R.verify_with_wait(REAL_SHA, 10 ** 12, LEDGER, wait_s=3, poll_s=99,
